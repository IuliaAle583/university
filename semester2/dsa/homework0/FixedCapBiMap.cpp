...
ValuePair FixedCapBiMap::removeKey(TKey k) {
	...
	ValuePair foundValues = this->search(k);

	if (foundValues.first == NULL_TVALUE) {
		return ValuePair(NULL_TVALUE, NULL_TVALUE);
	}

	this->remove(k, foundValues.first);
	if (foundValues.second != NULL_TVALUE) {
		this->remove(k, foundValues.second);
	}
