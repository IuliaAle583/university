...
private:
    Session& session;   //!!!!!!!
    Driver& driver;     //!!!!!!! one driver per one window
...
protected:
    void paintEvent(QPaintEvent* event) override {
        ...
        for (auto& report : reports) {
            if (report.getStatus()) {
                int x = static_cast<int>((report.getLatitude() + 180) * (width() / 360.0));
                int y = static_cast<int>((90 - report.getLongitude()) * (height() / 180.0));
